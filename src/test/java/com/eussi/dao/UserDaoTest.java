...
public class UserDaoTest extends BaseDaoTest {
	...
	@Test
	@DataSet("EussiWebsite.Users.xls")//准备数据
	...
	// 验证数据库保存的正确性
	@Test
	@ExpectedDataSet("EussiWebsite.ExpectedSaveUser.xls")// 准备验证数据
	public void saveUser()throws Exception  {
		...
		**/
		...
		User u  = XlsDataSetBeanFactory.createBean(UserDaoTest.class,"EussiWebsite.SaveUser.xls", "t_user", User.class);
		userDao.save(u);  //执行用户信息更新操作
	}
	
	//验证数据库保存的正确性
	//@Test
	//@ExpectedDataSet("EussiWebsite.ExpectedSaveUsers.xls")// 准备验证数据
	public void saveUsers()throws Exception  {
		List<User> users  = XlsDataSetBeanFactory.createBeans(UserDaoTest.class,"EussiWebsite.SaveUsers.xls", "t_user", User.class);
		for(User u:users){
		     userDao.save(u);
		}
	}
